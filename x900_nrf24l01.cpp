...
enum {
    ...
    FLAG_FLIP3     = 0x0100,    // flip na x800
    FLAG_LIGHT     = 0x0200,    // light na x800
    FLAG_UNK11     = 0x0400,
...
static const u8 freqs[][4] = {{0x35, 0x42, 0x3D, 0x0A},{0x43, 0x36, 0x3F, 0x0A}};
...
static void v202_init()
{
    ...
    XN297_SetRXAddr(rx_p1_addr, 5);
}

static void set_tx_id(u32 id)
{
  // set_tx_id("\xA0\x0F\x03\x10")
    u8 sum;
    tx_id[0] = (id >> 24) & 0xFF;
    tx_id[1] = (id >> 16) & 0xFF;
    tx_id[2] = (id >> 8) & 0xFF;
    ...
    // channels (before bind): 0x35, 0x42, 0x3D, 0x0A,
    // channels (after bind) : 0x43, 0x36, 0x3F, 0x0A,
    for (u8 i = 0; i < 16; ++i) {
                          ...
                          u16* flags, u16* led_blink_count)
{
    ...
    *aileron = a;//a < 0x80 ? 0x7f - a : a;
    ...
    int num_channels = 13; //Model.num_channels;


    if (Channels[CHANNEL5] <= 0) *flags &= ~ 0x01;
    else *flags |= 0x01;

    if (Channels[CHANNEL6] <= 0) *flags &= ~ 0x02;
    else *flags |= 0x02;

    if (Channels[CHANNEL7] <= 0) *flags &= ~ 0x04;
    else *flags |= 0x04;

    if (Channels[CHANNEL8] <= 0) *flags &= ~ 0x08;
    else *flags |= 0x08;

    if (Channels[CHANNEL9] <= 0) *flags &= ~ 0x10;
    else *flags |= 0x10;

    if (Channels[CHANNEL10] <= 0) *flags &= ~ 0x20;
    else *flags |= 0x20;

    if (Channels[CHANNEL11] <= 0) *flags &= ~ 0x0100;
    else *flags |= 0x0100;

    if (Channels[CHANNEL12] <= 0) *flags &= ~ 0x0200;
    else *flags |= 0x0200;
    /*
    // Channel 6
    if (Channels[CHANNEL6] <= 0) *flags &= ~FLAG_FLIP2;
    else *flags |= FLAG_FLIP2;

    // Channel 7
    if (num_channels < 7 || Channels[CHANNEL7] <= 0) *flags &= ~FLAG_LIGHT;
    else *flags |= FLAG_LIGHT;

    // Channel 8
    if (num_channels < 8 || Channels[CHANNEL8] <= 0) *flags &= ~FLAG_UNK00;
    else *flags |= FLAG_UNK00;

    // Channel 9
    if (num_channels < 9 || Channels[CHANNEL9] <= 0) *flags &= ~FLAG_FLIP3;
    else *flags |= FLAG_FLIP3;
    ...
    // Channel 10
    if (num_channels < 11 || Channels[CHANNEL11] <= 0) *flags &= ~FLAG_UNK11;
    else *flags |= FLAG_UNK11;

    // Channel 11
    if (num_channels < 12 || Channels[CHANNEL11] <= 0) *flags &= ~FLAG_FLIP;
    else *flags |= FLAG_FLIP;

    // Channel 12
    if (num_channels < 13 || Channels[CHANNEL11] <= 0) *flags &= ~FLAG_VIDEO;
...
static void send_packet(u8 bind)
{
    ...
    u8 rf_ch = freqs[phase == V202_DATA ? 1 : 0][rf_ch_num >> 1];
    rf_ch_num = (rf_ch_num + 1) & 0x07;
...
const void *X900_Cmds(enum ProtoCmds cmd)
{
    switch(cmd) {
        case PROTOCMD_INIT:
