...
enum {
    ...
    FLAG_CAMERA = 0x400000, // also automatic Missile Launcher and Hoist in one direction
    FLAG_VIDEO  = 0x800000, // also Sprayer, Bubbler, Missile Launcher(1), and Hoist in the other dir.
                          ...
                          u32* flags, u16* led_blink_count)
{
    ...
    *aileron = Channels[CHANNEL1]; //a < 0x80 ? 0x7f - a : a;
    ...
    int num_channels = 10;//Model.num_channels;
    ...
    // Channel 9
    if (num_channels < 9 || Channels[CHANNEL9] <= 0) {
      *flags &= ~FLAG_HEADLESS;
      *flags |= FLAG_NORMAL;
    }
    else {
...
static void send_packet(u8 bind)
{
    ...
    switch (bind) {
        ...
        case 0:          
          ...
          packet[4] = (flags >> 16) & 0xff;
          packet[5] = 0x05 | ((flags >> 8) & 0xff) ; 
          packet[6] = flags & 0xff;
          packet[7] = 0x21;
          packet[8] = 0x00;
          break;
