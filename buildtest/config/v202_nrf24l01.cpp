...
static const u8 freq1[2][8] = {{0x07, 0x24, 0x3e, 0x2b, 0x47, 0x0e, 0x39, 0x1c},{0x0f, 0x2a, 0x4b, 0x21, 0x2d, 0x36, 0x11, 0x35}};
...
static void v202_init()
{
    ...
    //NRF24L01_WriteRegisterMulti(0x1f, (u8 *) "\xd1\x3f\x7f\x9c\x20", 5);
    //NRF24L01_WriteRegisterMulti(0x1e, (u8 *) "\xda\x9a\xb0\x79\xbb\xab\x9c", 7);
    //NRF24L01_WriteRegisterMulti(0x19, (u8 *) "\x0b\xdf\xc4\xa7\x03", 5);
    //NRF24L01_WriteRegisterMulti(NRF24L01_10_TX_ADDR, (u8 *) "\x68\x94\xa6\xd5\xc3", 5);
    XN297_SetTXAddr((u8 *) "\x68\x94\xa6\xd5\xc3", 5);
    ...
    NRF24L01_WriteReg(NRF24L01_11_RX_PW_P0, 0x10); // packet size
...
static void send_packet(u8 bind)
{
    if (bind) {
        ...
        //for(int i=0; i<4; i++) rf_channels[i] = freq1[1][i];
        // regular packet
...
//        phase = V202_BIND1;
        for(int i=0; i<8; i++) rf_channels[i] = freq1[0][i];
        phase = V202_BIND1;
        return 150;
        break;
    ...
    case V202_BIND1:
        send_packet(1);
        if (--counter == 0) {
        for(int i=0; i<8; i++) rf_channels[i] = freq1[1][i];
        XN297_SetTXAddr((u8 *) "\x2b\xa5\x37\xc5\x4a", 5);
        NRF24L01_WriteReg(NRF24L01_06_RF_SETUP, 0x07);
        phase = V202_BIND2;
        phase = V202_DATA;
