...
#define PACKET_PERIOD        8000     // Timeout for callback in uSec
...
static u8 tx_power;
static u8 throttle, rudder, elevator, aileron, flags, elevator_trim, aileron_trim;
...
static void read_controls(u8* throttle, u8* rudder, u8* elevator, u8* aileron, u8* flags, u8* elevator_trim, u8* aileron_trim)
{
...
//    *throttle = *throttle & 0x80 ? 0xff - *throttle : 0x80 + *throttle;
    //*throttle = *throttle & 0x80 ? 0xff - *throttle : *throttle;
//    *rudder   = convert_channel(CHANNEL4);

    *aileron  = Channels[CHANNEL1];
    *elevator = Channels[CHANNEL2];
    *throttle = Channels[CHANNEL3];
    *rudder   = Channels[CHANNEL4];
    *elevator_trim = Channels[CHANNEL5];
    *aileron_trim = Channels[CHANNEL6];
...
static void build_packet(u8 bind) {
    if (bind) {
        ...
        read_controls(&throttle, &rudder, &elevator, &aileron, &flags, &elevator_trim, &aileron_trim);
...
#ifdef EMULATOR
    //dbgprintf("pkt %d: chan 0x%02x, bind %d, data %02x", packet_counter, chans[current_chan], bind, packet[0]);
...
static void symax_init()
{
    ...
    //const u8 bind_rx_tx_addr[] = {0x9a,0x20,0x08,0x00,0xa2};
...
//      memcpy(chans, chans_all_test, num_rf_channels);
    current_chan = 0;
    packet_counter = 0;
}
